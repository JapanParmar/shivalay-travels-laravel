...
@section('page_title', 'Villas Inventory')
@section('page_subtitle')
    {{ count($villas) }} villas in local database
@endsection
...
<div class="gd-root">
    ...
    <div class="gd-header">
        <div></div>
        <button class="gd-add-btn" id="toggleAddBtn">+ Add Villa</button>
    </div>

    <!-- Form (Creates or Edits) -->
    <div class="gd-form-card" id="villaForm" style="display: none;">
        <h3 class="form-card-title" id="formTitle">Add New Villa</h3>
        <form action="/admin/villas" method="POST">
            @csrf
            <input type="hidden" name="editing_id" id="editingId" value="" />
            <div class="form-grid">
                <div class="form-group">
                    <label class="form-lbl">Villa Name *</label>
                    <input type="text" name="name" id="inName" class="form-input" placeholder="e.g. Whispering Pines Luxury Villa" required minlength="3" maxlength="150" />
                </div>
                <div class="form-group">
                    <label class="form-lbl">Location *</label>
                    <input type="text" name="location" id="inLocation" class="form-input" placeholder="e.g. Gulmarg, Kashmir" required minlength="3" maxlength="150" />
                </div>
                <div class="form-group">
                    <label class="form-lbl">Starting Price *</label>
                    <input type="text" name="price" id="inPrice" class="form-input" placeholder="e.g. ₹12,500 / night" required minlength="2" maxlength="50" />
                </div>
                <div class="form-group">
                ...
                <div class="form-group full-width">
                    ...
                    <textarea name="amenities" id="inAmenities" rows="2" class="form-textarea" placeholder="e.g. Private Pool, Personal Chef, Heated Bedrooms" required minlength="5"></textarea>
                </div>
                <div class="form-group full-width">
                    <label class="form-lbl">Description *</label>
                    <textarea name="description" id="inDescription" rows="4" class="form-textarea" placeholder="Describe the villa details, view, capacity, amenities..." required minlength="10"></textarea>
                </div>
            </div>

            <div class="form-actions">
                <button type="button" class="gd-cancel-btn" id="cancelBtn">Cancel</button>
                <button type="submit" class="gd-save-btn" id="submitBtn">Save Villa</button>
            </div>
        </form>
    </div>
    ...
    <div class="gd-table-wrap">
        <table class="gd-table datatable-enabled">
            ...
            <tbody>
                @foreach($villas as $v)
                    <tr>
                        <td>
                            <img src="{{ $v['imagePath'] }}" alt="{{ $v['name'] }}" style="width: 80px; height: 50px; object-fit: cover; border-radius: 6px; border: 1px solid rgba(255,255,255,0.08);" />
                        </td>
                        <td>
                            <div class="guide-title-cell">
                                <span class="guide-title-text">{{ $v['name'] }}</span>
                                <span class="guide-title-sub" style="max-width: 250px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ $v['description'] }}</span>
                            </div>
                        </td>
                        <td>{{ $v['location'] }}</td>
                        <td><span style="color: var(--color-highlighter-lime, #22c55e); font-weight: 600;">{{ $v['price'] }}</span></td>
                        <td>⭐ {{ $v['rating'] }}</td>
                        <td>
                        ...
                        <td style="text-align: right;">
                            <div class="action-buttons">
                                <button class="edit-btn" data-villa="{{ json_encode($v) }}" onclick="editVilla(JSON.parse(this.getAttribute('data-villa')))">Edit</button>
                                <a href="/admin/villas/delete/{{ $v['id'] }}" class="delete-btn" onclick="return confirm('Delete this villa?');" style="text-decoration: none;">Delete</a>
                            </div>
                        </td>
                    </tr>
                @endforeach
        ...
        @if(count($villas) === 0)
            <div class="gd-empty">No villas found in the database. Add one to get started!</div>
        @endif
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (toggleAddBtn) {
        toggleAddBtn.addEventListener('click', function() {
            resetForm();
            formTitle.innerText = 'Add New Villa';
            submitBtn.innerText = 'Save Villa';
            villaForm.style.display = villaForm.style.display === 'none' ? 'block' : 'none';
        });
    }
    ...
    window.editVilla = function(v) {
        document.getElementById('editingId').value = v.id;
        document.getElementById('inName').value = v.name || '';
        document.getElementById('inLocation').value = v.location || '';
        document.getElementById('inPrice').value = v.price || '';
        document.getElementById('inRating').value = v.rating || '';
        document.getElementById('inAmenities').value = (v.amenities || []).join(', ');
        document.getElementById('inDescription').value = v.description || '';

        const mainImg = v.imagePath || '';
        const galleryArray = v.gallery || [];
        ...
        formTitle.innerText = 'Edit Villa';
        submitBtn.innerText = 'Save Changes';
        villaForm.style.display = 'block';
