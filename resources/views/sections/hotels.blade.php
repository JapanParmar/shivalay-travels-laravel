<!-- Hotels Section -->
<section id="hotels" style="background: var(--surface-canvas); padding: 80px 0; border-bottom: 1px solid var(--color-zinc-hairline);">
  <div class="container">
    <!-- Header -->
    <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 40px; flex-wrap: wrap; gap: 20px;">
      <div>
        <p class="font-primary text-xs fw-medium uppercase ls-2 text-muted" style="margin-bottom: 8px;">✦ Handpicked Accommodations</p>
        <h2 class="font-secondary fs-36 fw-regular" style="color: var(--color-pure-white); line-height: 1.1;">Premium Hotels &amp; Retreats</h2>
      </div>
      <p class="font-primary text-sm text-muted" style="max-width: 360px;">
        Curated premium stays with verified amenities and exclusive agency-only tariffs.
      </p>
    </div>

    <!-- Grid -->
    <div class="hotels-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 24px;">
      @foreach($hotels as $h)
        <div class="hotel-card" style="background: var(--color-carbon); border: 1px solid var(--color-zinc-hairline); border-radius: var(--radius-xl); overflow: hidden; display: flex; flex-direction: column; transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1);"
          ...
          <!-- Image Carousel -->
          @php
            $gallery = array_filter(array_merge([$h['imagePath'] ?? null], $h['gallery'] ?? []));
          @endphp
          ...
          <div style="padding: 24px; flex: 1; display: flex; flex-direction: column; gap: 16px;">
            <div>
              <h3 class="font-secondary fs-20 fw-regular" style="color: var(--color-pure-white); margin-bottom: 8px;">{{ $h['name'] }}</h3>
            ...
            <div style="border-top: 1px solid rgba(255,255,255,0.06); padding-top: 16px; display: flex; align-items: center; justify-content: space-between; margin-top: auto;">
              <div>
                ...
                <span class="font-primary fs-18 fw-medium" style="color: var(--color-highlighter-lime);">{{ $h['price'] }}</span>
              </div>
              <button type="button" onclick='window.openPropertyModal({!! json_encode($h) !!}, "hotel")' class="btn-primary" style="padding: 8px 16px; font-size: 12px; border: none; cursor: pointer;">
                Book Now →
              </button>
            </div>
          </div>
